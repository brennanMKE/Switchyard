...
struct DescriptionCoverageTests {
    ...
    private func registry() throws -> [Row] {
        ...
        return [
            ...
            Row("Merge.swift", "MergeError",
                MergeError.unrelatedHistories(branch: "probe-branch"),
                .carries(["unrelated histories", "probe-branch", "nothing was touched"])),
            Row("LaneAssignment.swift", "RevListParser.Failure",
                RevListParser.Failure.malformedLine("probe-not-hex"),
                .carries(["probe-not-hex"])),
            Row("RefManage.swift", "RefManageError",
                RefManageError.alreadyExists(kind: .tag, name: "probe-tag"),
                .carries(["a tag named 'probe-tag' already exists", "nothing was touched"])),
            Row("RefManage.swift", "RefKind",
                RefKind.branch,
                .carries(["branch"])),
            ...
            Row("Replay.swift", "ReplayError",
                ReplayError.unknownCommit("probe-revision"),
                .carries(["probe-revision", "nothing was touched"])),
            Row("Rewrite.swift", "RewriteError",
                RewriteError.unknownCommit("probe-revision"),
                .carries(["probe-revision", "nothing was touched"])),
            Row("RewriteDiff.swift", "RewriteDiffError",
                RewriteDiffError.unparseableOutput(detail: "probe drift detail"),
                .carries(["probe drift detail"])),
            Row("Rerere.swift", "RerereError",
                RerereError.unexpectedCacheEntry(name: "probe-not-an-id"),
                .carries(["rr-cache", "probe-not-an-id"])),
            Row("RerereForget.swift", "RerereForgetError",
                RerereForgetError.nothingRecorded(paths: ["probe/f.txt"]),
                .carries(["no recorded resolution", "probe/f.txt"])),
            Row("ReviewNotes.swift", "ReviewNotes.Error",
                ReviewNotes.Error.malformedNotesListLine("probe notes line"),
                .carries(["probe notes line"])),
            Row("Split.swift", "SplitError",
                SplitError.unknownHunkID(id: "h-probe-hunk"),
                .carries(["h-probe-hunk", "nothing was touched"])),
            Row("Squash.swift", "SquashError",
                SquashError.mergeRefused(commit: "probe-commit"),
                .carries(["probe-commit", "second parent's line of history"])),
            ...
            Row("ResolveApply.swift", "ResolveApplyError",
                ResolveApplyError.pathNotConflicted(path: "probe/conflicted.txt"),
                .carries(["probe/conflicted.txt", "no unmerged entries"])),
