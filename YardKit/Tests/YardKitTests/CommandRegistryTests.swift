...
@Test("registry reports exactly ten entries")
func registryHasExactlyTenEntries() {
    #expect(CommandRegistry.all.count == 10)

    let names: [String] = CommandRegistry.all.map(\.name)
    #expect(Set(names).count == 10, "Names must be distinct so lookup returns the right spec.")
}
