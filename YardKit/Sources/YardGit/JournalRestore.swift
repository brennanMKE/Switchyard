...
public enum JournalRestore {
    ...
    static func restoreAssumingLock(
        ...
        // 8b. Restore the sequencer state, if captured. The tree is stored
        // as a single tree object in the anchor; restore materialises it into
        // the sequencer directory byte-for-byte.
        if metadata.captured.sequencer != .notCaptured,
           let oid = slots[JournalAnchor.sequencerTreeEntryName] {
            let layout: SequencerSnapshot.Layout = switch metadata.captured.sequencer {
            case .merge: .rebaseMerge
            case .notCaptured: fatalError("sequencer not captured but we are restoring it")
            }
