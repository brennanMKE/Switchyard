...
Review: I re-ran the YardUI bundle (264 passed). Both mutations kill a named test: the deleted check
rewritten to look for `new file mode` fails `deletedFileHeaderIsDeleted`, and `prefix(7)` fails
`titleIsShortOidThenSubject`. I read all five new tests. The squash is on `main` as `ed005fc`.
The full suite ran twice on merged `main` from `../sy-check` and was green both times, 1,975 tests.
