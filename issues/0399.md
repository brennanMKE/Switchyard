...
## Review — round 1, 2026-09-22: failed (spec defect, mine)

The round applied the plan faithfully: the package suite was green and the UI-test target built.
The **VM suite failed 4 of 10**: 0382, 0383, 0385 and 0399, each with
`Failed to get matching snapshots: Timed out while evaluating UI query`. Both causes were in the
plan (`docs/review-failures.md`, #0399 r1):

1. **Chipless rows vanished from the accessibility tree.** With every `Text` gone,
   `.accessibilityElement(children: .combine)` has nothing to combine, and SwiftUI emits no
   element despite the `.accessibilityLabel`. The VM hierarchy showed the tip and second commits
   (which have chips) as `StaticText`, and the third and root commits as bare empty `Cell`s.
   That's a VoiceOver regression, not only a test break.
2. **`descendants(matching: .any)` + `value CONTAINS`** evaluates `CONTAINS` against numeric
   values (disclosure triangles, the splitter). It throws inside the app and retries until the
   query times out.

**Measured fix (VM, 2026-09-22, via `build/uitest-overlay/`):** with the two changes below,
`./scripts/run-ui-tests-vm.sh 0399` passed: Smoke 3.6 s, 0399 4.4 s, `TEST SUCCEEDED (exit 0)`.

## Round 2 — literal changes

1. **`YardKit/Sources/YardUI/CommitHistoryView.swift`**, in `CommitHistoryRow.body`, replace
...
2. **`SwitchyardUITests/UITestSupport.swift`**: revert round 1's helper change. In
   `historyRows(containing:)`, change `return descendants(matching: .any).matching(NSPredicate(`
   back to `return staticTexts.matching(NSPredicate(`. The predicate stays unchanged.

Nothing else changes. The rest of round 1 (the chips-only body, `rowHeight`,
`defaultMinListRowHeight`, the 0399 test and the script line) stays.

### Round 2 verification

1. `cd YardKit && swift test`: counts equal to round 1's (162/267/373/1045/14/117), all passing.
2. From the worktree root:
   `xcodebuild build-for-testing -project Switchyard.xcodeproj -scheme 'Switchyard UITests' -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO -derivedDataPath build/dd -quiet`
   has no `error:`. No UI tests on this Mac. The reviewer runs the full VM suite.

- **2026-09-22, round 2 (Sonnet):** 62,664 subagent tokens, 167 s wall. Applied both literal
  changes; package suite 162/267/373/1045/14/117 green; UI-test target built. Committed `c0d8b5f`.

## Resolved — 2026-09-22 (the #0399 row change; #0400 carries the label work)

Review:
- **VM suite, round 2:** all 12 tests passed, including every earlier spike test and
  `Spike0399CompactGraphRowsUITests`. The summary now prints its `---` headers and exits 0,
  confirming #0407 end to end.
- **Mutation, killed:** overlaying the pre-#0399 `CommitHistoryView.swift` (two-line rows) fails
  the 0399 test at 39 pt against the 26 pt limit.
- **Mutation, survived (recorded honestly):** restoring a single `Text(entry.subject)` in the row
  still passes. One line fits in 22 pt, and `children: .ignore` hides inner text from
  accessibility, so the test can't see text that is present. The test pins compactness, not text
  absence. Text absence shows in the #0400 VM screenshot.
- **Merge:** with `main` merged into the branch the package suite was green. The squash is on
  `main` as `382ba95`. Merged `main` then ran twice: run 1 green, run 2 green except the #0408
  reaper flake, and the YardUI bundle re-ran green (267).
