...
| `scripts/list-issues-by-milestone` | working a milestone goal | guessing at progress. Prints each milestone's completion percentage plus its open / in-progress counts, which is the goal's whole state. `-m M2` for one milestone, `--open` to hide finished work. |
