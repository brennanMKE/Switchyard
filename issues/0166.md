...
**Known defect, still resolved.** The implementation (`0173f4c`) predates the reset
and is still on `main`. The later fix to its `JournalObserved` JSON key pinning and its
chain test (`aef78c5`) is now only on `salvage/main-off-workflow`, and returns with #0153.
