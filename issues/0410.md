...
| **Commits** | `acbd48f` (#0411), `f9b0a61` (#0412), `3610f5e` (#0413), `4dac184` (#0414), `ae847e5` (#0415) |
...
- [x] Opening a repository shows every local branch's tip on one top row, with its name visible,
      without scrolling (horizontal scrolling is acceptable when there are more columns than fit).
- [x] Each branch's own commits run down its column; a connector reaches its fork point.
- [x] Selecting a node still drives the Detail pane; double-click still opens the changes window
      (#0406); the filter still dims non-matches (#0402); clicking a sidebar branch still
      focuses it (#0401).
- [x] Every behaviour has a VM UI test, and a VM screenshot of a multi-branch fixture shows the
      aligned tips.
...
- 2026-09-23 planning prototype (worktree `sy-plan-0410`, detached, not pushed): the full chain
  #0411-#0415 wired in and measured in the VM. Run a: the 2-D scroll view opened centred
  horizontally, with the map starting at `lane-13`, and the node targets showed up as accessibility
  **Buttons** because of the tap gestures, so `staticTexts` found none. Fixed with
  `.defaultScrollAnchor(.topLeading)` and `.accessibilityRemoveTraits(.isButton)`. Run b: lane
  labels are found through `label` **or** `value`. Run d: the map test passed. Then the full VM
  suite ran against the prototype with `RESULT: TEST SUCCEEDED`, 11 spikes, including 0382 (no
  double firing from the per-node context menus), 0383 (with the head-lane migration) and
  0399/0401/0402/0403/0406 unchanged. Host after the runs: no leftover clones. The new crash
  reports were all `Curator-*.ips` from another project's session, none from Switchyard or tart.
- 2026-09-23: all five children resolved, each in one Sonnet round, each byte-identical to the
  planning prototype. On `main` the full VM suite passes (11 spikes); selection → Detail (0401,
  0403), double-click → changes window (0406), filter dimming + ⌘G (0402), sidebar focus (0401,
  0415) and ⌥⌘↑/↓ (0383) all pass on the map.

![VM: the branch map on the multi-branch fixture](0415/vm-branch-map.png)

## Resolved

Children #0411-#0415 merged. Every labelled lane's tip sits on the top row under its slanted label.
On Batty (measured by `BranchMapLayout`) that is 121 of 121 non-stub tips.

**Not verified on Batty itself.** No UI test runs on the host, and the VM has no Batty. Brennan
should open Batty and look. Follow-ups worth considering, not filed as defects:
- git/git produces 122 unlabelled tracks, which is wide.
- Tags have no lane.
- Unmodified ↑/↓ step within a lane but have no VM test.
- ⌘C (Edit ▸ Copy) on the map has no VM test.
