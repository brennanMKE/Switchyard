...
- [ ] `swift test` run from `YardKit/` is green and its summary line reads
      `Test run with 692 tests` — N must exceed the baseline on `main`
      (`docs/test-baseline.txt`, currently **684**) by **exactly 8**. 554 + 8 = 562 was measured on the
...
(the baseline was 554 at that measurement; it is 684 now — **trust the +8 delta, not this absolute**)
...
# must print: Test run with 692 tests … passed   (baseline 684 + 8 new)
```
