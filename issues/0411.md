...
- 2026-09-23 round 1 (Sonnet, 577 s wall, 87,646 subagent tokens): both files transcribed; the
  reviewer diffed them against the planning prototype and found them byte-identical, so the six
  mutations applied before filing (all red) cover these exact bytes. Review re-ran `swift test` in
  the worktree with `main` merged: 162/291/373/1045/14/117, all passing (YardUITests 278 → 291, +13).
  UI-test build clean.
- Merged main, full suite twice in a throwaway worktree: green, then one #0408 flake
  (`cancellingAsyncCaptureTerminatesTheChildAndThrows`), then green on re-run.

## Resolved

Squash-merged as `acbd48f`. `BranchMapLayout.make(rows:refs:)` is on `main` and nothing uses it yet.
#0412 builds on it.
