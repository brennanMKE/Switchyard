...
|---|---|---|---|
| Planning pass (re-author, split, measure) | Fable 5 | 189,586 | $3.03 |
| Round 1 (accepted, no hand finish) | Opus 5 dispatcher | 58,592 | $0.47 |
| Implementation | Ornith 1.0 35B-A3B, local via LM Studio | 342,871 (333,470 in / 9,401 out) | **$0.00** |
| **Total billed** | | **248,178** | **$3.50** |

**One round, ~5 minutes of model time, 9 tool calls, first write at call 4, zero edits.** Both files
created with `cat > … <<'SWIFT'` heredocs, no `write` tool call at all — a 365-line file went through
without touching the schema that produced `SchemaError(Missing key at ["content"])` on #0014. All 8
mutations were applied individually and every one turned its named test red, two of them reproducing
the planner's predicted error strings verbatim.

**No compaction**, on the largest issue dispatched to date (~23.8k of a ~44k budget). The round never
revisited the issue text, so context never grew past the initial load. Suite 325 -> 341 on the branch,
384 on `main`.

### What the planning pass caught, and it was the whole issue

The old text specified a test in the **vacuous direction**: stage the *second* of two hunks, then
re-list. Measured, staging the second hunk shifts nothing before it — so that test passes even against
a position-derived id, which is exactly the defect stable ids exist to prevent. It would have certified
the bug.

The re-authored test stages the **first** hunk and asserts the second's id survives its own header
moving from `@@ -14,7` to `@@ -15,7` with its body byte-identical. That single measurement is what the
whole design rests on.

Four other corrections: "IDs are invalidated — detectably, with a clear error" is unimplementable in a
*listing* function, since there is no id argument to reject (it belongs to #0040); `Module` named a
module that does not exist; "blocked by #0009, #0011" was stale in both directions; and edits six or
fewer lines apart **merge into one hunk** at `-U3`, so a fixture that does not account for it produces
one hunk where the test expects two.

The split needed no new issue — #0040 already covers staging by hunk id.

Reviewed by re-running the suite and applying an independent mutation not in the issue's table:
dropping the NUL separator between path and body in `hunkID`, which failed
`hunkIdIsAPureFunctionOfPathAndBody` against its pinned vector `b88468cda3e3`. Source restored
byte-identical.

### The harness cost that was not the round's fault

The dispatcher then sat in `await-dispatch.sh` for roughly 25 minutes after this round had already
finished, because `pgrep` kept matching a stale wrapper argv. Fixed in the same session; see the
failure log.

## A defect found later, filed as #0161

While planning **#0040** (staging by hunk id), a parser defect in this issue's shipped code was
measured: `git diff --cached` prints `* Unmerged path <p>` at its sorted position among the diffs, and
`HunkParser` appends that line to the **previous** file's `headerText`. The reconstructed patch then
fails `git apply` with `patch with only garbage at line 5`.

It does not affect listing, which is what this issue delivered and what its tests cover — it surfaces
only when the reconstructed patch is fed back to git, which nothing did until #0040. Hardening is
**#0161**, with the cross-file reproduction in #0162's mutation MB4.

Worth recording here rather than only there: this issue's tests were mutation-verified and its parser
was measured against real diff output, and the gap was still real. Conflicted paths were simply never
in the sample — `#0017` owned conflicts, and the two issues' test corpora did not overlap.
