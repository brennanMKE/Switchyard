...
- 2026-09-23 round 1 (Sonnet, 604 s wall, 89,981 subagent tokens): the fixture script matches the
  prototype byte for byte; the other edits match it apart from the 0410 → 0415 rename. The fixture
  check printed 29 branches. Package counts are unchanged.
- Review: the full VM suite from `6756ad8` ended `RESULT: TEST SUCCEEDED`, 11 spikes, including
  `Spike0415BranchMapUITests` (16.1 s). Host after the run: no leftover clones, and no new crash
  reports. Screenshots:

![VM: the branch map on the multi-branch fixture, every tip on the top row](0415/vm-branch-map.png)

![VM: after clicking merged-old in the sidebar, its tip 30 rows down is centred and selected](0415/vm-branch-map-scrolled.png)

- After the squash, two full runs on merged `main` were both green: 162/307/373/1045/14/117.

## Resolved

Squash-merged as `ae847e5`.
