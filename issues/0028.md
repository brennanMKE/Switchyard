...
of `ee66571`, plus #0027's three files transcribed from its issue) on 2026-08-07** — the suite
measured **556 before and `Test run with 571 tests in 38 suites` after, delta +15 exactly**, and
...
11. **Baseline.** `docs/test-baseline.txt` reads **554** as this pass finishes (main, including
    #0029, without #0027). The scratch tree — `git archive` of `ee66571`, which predates #0029's
    merge and measured 545, **plus #0027's three files** (+11) — measured **556** with these
    files absent, **571 in 38 suites** with them present: delta **+15** exactly (14 in
    ...
    on a moved baseline that includes at least #0027 and #0029 (554 + 11 = 565 before these
...
- [ ] Round 1: `cd YardKit && swift build` succeeds with only Change 1 present. Round 2:
      ...
      exactly 15**. (Scratch tree: 556 → 571; the branch baseline will differ — trust the delta.)
