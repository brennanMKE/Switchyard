...
## Review — after round 1, read this first
...
1. Create `YardKit/Tests/YardGitTests/JournalAnchorTests.swift` from the Change 2 block below,
   verbatim.
...
3. Run `cd YardKit && swift test` and paste the real `Test run with N tests` summary line. The
   branch measures **565 before these two files**; expect **580** after — a delta of exactly
   **+15**. (The Description's "582" is a typo for the absolute; the **+15 delta** is the number
   that was measured and is what to check.)

**Do not open, edit, or rewrite `YardKit/Sources/YardGit/JournalAnchor.swift`.** It is done. If a
test does not compile, fix the test file to match the source that exists — do not change the
source. Do not touch `YardKit/Package.swift`; both test targets already exist.
