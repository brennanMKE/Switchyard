...
6. **The suite in the scratch tree (cut at `73dead0`) with both files present: `Test run
   with 620 tests in 43 suites passed`**, against 610 in the same tree without them — the
   delta is exactly the 10 new tests. The delta is the criterion; the absolute moves under
   every planned issue on this project (it has again during this pass, 610 → 618).
...
- `swift test` prints a passing `Test run with N tests` line whose N equals the branch's
  pre-change count — `docs/test-baseline.txt` reads **629** at dispatch (it read 618 at
  authoring; the absolute has moved six times today), and round 1 adds no
  tests, so the count must not move and must not fail. Trust the delta, not this absolute.
...
- `swift test` prints a `Test run with N tests` line where **N must exceed the round-1 count
  by exactly 10** — 618 → 628 if the baseline has not moved again — and the run passes.
  (Measured 610 → 620 in the scratch tree; the delta is the criterion.)
...
- [ ] `YardKit/Tests/YardGitTests/JournalEntryMetadataTests.swift` exists, transcribed from
      Change 2; the suite count rises by exactly 10 over the round-1 count and passes.
...
**Round 1 accepted — nothing went wrong.** This heading exists because the harness requires one for
any round after the first, and the two-round split here is by design (issue preamble: "Two rounds by
design: Change 1 is round 1, Change 2 is round 2"), not a repair.

What round 1 landed, verified against the diff:

- `YardKit/Sources/YardGit/JournalEntryMetadata.swift`, 382 lines, transcribed from the Change 1
  block. Diffed against the block extracted from this file: the only difference is two blank lines
  removed (before `IndexCapture.init(from:)` and before `WorktreeCapture.init(from:)`). No semantic
  drift.
- `swift build` clean; `Test run with 629 tests in 44 suites passed` — the branch's pre-change count,
  unmoved, as round 1 adds no tests. Baseline 629 re-measured on this branch before the round.

**Round 2 is Change 2 and nothing else**: create
`YardKit/Tests/YardGitTests/JournalEntryMetadataTests.swift` and transcribe the Change 2 source
block exactly. Do **not** edit `JournalEntryMetadata.swift` — it is correct and already reviewed. Do
not edit `JournalChain.swift`. The verification is the round-2 verification above: `swift test` must
print a passing `Test run with N tests` line with **N = 639** (629 + the 10 new tests; the 618/628
figures in the round-2 verification text are older scratch numbers — the delta of exactly 10 is the
criterion), and `git status --short` must list exactly the one new test file.
