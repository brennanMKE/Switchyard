...
- 2026-09-23 round 1 (Sonnet, 1,061 s wall, 77,933 subagent tokens): both files byte-identical to
  the planning prototype, so the six mutations applied before filing (all red) cover these bytes.
  Worktree with `main` merged: 162/304/373/1045/14/117. YardUITests went 291 → 304 (+13) and was
  green in the round. In review it was red once, on the #0408 reaper flake only.
- Merged `main`, three full runs in a throwaway worktree while the host load average was 70-95.
  Every run was red **only** on known or load-sensitive tests in bundles this issue does not touch:
  run 1 on the reaper flake; run 2 on the reaper plus `cancellingAsyncCapture…`; run 3 on
  `craftedBadSSHSignatureReportsBad` (git exited 13, SIGPIPE), which is recorded on #0408. The
  `BranchMapGeometryTests` passed in every run. Accepted on that basis.

## Resolved

Squash-merged as `f9b0a61`.
