...
### The critical constraint: `switchyard` must work without the app

This is the single most important architectural decision in the project.

If `switchyard` requires Switchyard.app to be running, the CLI is useless in CI, over SSH, and in
...
- Add `switchyard --require-app` to fail fast, and `switchyard <cmd> --no-launch` matching RemoteControl's
